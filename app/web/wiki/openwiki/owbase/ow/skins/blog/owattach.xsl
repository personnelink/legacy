...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ow="http://openwiki.com/2001/OW/Wiki" xmlns="http://www.w3.org/1999/xhtml" extension-element-prefixes="msxsl ow" exclude-result-prefixes="" version="1.0">
		...
		<xsl:if test="$showhidden='true' or count(ow:attachment[@hidden='false' and @deprecated='false']) &gt; 0">
			...
			<table cellspacing="0" cellpadding="2" border="0" width="100%">
				...
				<xsl:apply-templates select="ow:attachment">
					<xsl:with-param name="showhidden">
						<xsl:value-of select="$showhidden"/>
					</xsl:with-param>
					<xsl:with-param name="showactions">
						<xsl:value-of select="$showactions"/>
					</xsl:with-param>
				</xsl:apply-templates>
			</table>
		</xsl:if>
		...
		<xsl:if test="$showhidden='true' or (@hidden='false' and @deprecated='false')">
			<tr bgcolor="#ffffff" valign="top">
				<td nowrap="nowrap" class="fileattr">
					<a href="{ow:file/@href}" target="_blank">
						<img src="{/ow:wiki/ow:location}{/ow:wiki/ow:iconpath}/doc/{ow:file/@icon}.gif" border="0" hspace="2" width="16" height="16"/>
					</a>
				</td>
				<td nowrap="nowrap" class="fileattr">
					<a href="{ow:file/@href}">
						<xsl:value-of select="ow:file"/>
					</a> (v<xsl:value-of select="@revision"/>)&#160;
            </td>
				<td nowrap="nowrap" align="right" class="fileattr">
					<xsl:value-of select="ow:file/@size"/> KB&#160;&#160;&#160;</td>
				<td nowrap="nowrap" class="fileattr">
					<xsl:value-of select="ow:formatShortDateTime2(string(ow:date))"/>&#160;&#160;</td>
				<xsl:choose>
					<xsl:when test="ow:by/@alias">
						<td align="left" class="fileattr">
							<a>
				...
				<td class="fileattr">
					<xsl:if test="@deprecated='true'">
						<font color="#ff0000">
							<b>This file will be permanently destroyed.</b>
						</font>
						<xsl:if test="ow:comment">
							<br/>
						</xsl:if>
					</xsl:if>
					<xsl:value-of select="ow:comment"/>&#160;</td>
                ...
                <xsl:if test="$showactions='true'">
						<xsl:choose>
							...
							<xsl:when test="@hidden='true'">
								<a href="{/ow:wiki/ow:scriptname}?p={$name}&amp;a=undohidefile&amp;file={ow:file}&amp;rev={@revision}" title="make visible on wikipage">
									<img src="ow/images/hidden_on.gif" border="0" hspace="2" width="12" height="12" alt="make visible on wikipage"/>
								</a>
							</xsl:when>
							<xsl:otherwise>
								<a href="{/ow:wiki/ow:scriptname}?p={$name}&amp;a=hidefile&amp;file={ow:file}&amp;rev={@revision}" title="hide file from wikipage">
									<img src="ow/images/hidden_off.gif" border="0" hspace="2" width="12" height="12" alt="hide file from wikipage"/>
								</a>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:choose>
							<xsl:when test="@deprecated='true'">
								<a href="{/ow:wiki/ow:scriptname}?p={$name}&amp;a=undotrashfile&amp;file={ow:file}&amp;rev={@revision}" title="restore file">
									<img src="ow/images/undo.gif" border="0" hspace="2" width="16" height="12" alt="restore"/>
								</a>
							</xsl:when>
							<xsl:otherwise>
								<a href="{/ow:wiki/ow:scriptname}?p={$name}&amp;a=trashfile&amp;file={ow:file}&amp;rev={@revision}" title="trash file">
									<img src="ow/images/delico.gif" border="0" hspace="4" width="12" height="12" alt="trash"/>
								</a>
							</xsl:otherwise>
						</xsl:choose>
						<a href="{/ow:wiki/ow:scriptname}?p={$name}&amp;a=attachchanges&amp;file={ow:file}" title="view other revisions">
							<img src="ow/images/revs.gif" border="0" width="12" height="12" alt="view other revisions"/>
						</a>
					</xsl:if>
				</td>
			</tr>
	...
	<xsl:template match="ow:attachment">
		<xsl:choose>
			<xsl:when test="ow:file/@image='true'">
				<img src="{ow:file/@href}">
					<xsl:attribute name="title">Last changed: <xsl:value-of select="ow:formatLongDate(string(ow:date))"/></xsl:attribute>
				</img>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="ow:attachmentchange">
		<div align="right" class="fileattr">
            ...
            <xsl:choose>
				<xsl:when test="ow:by/@alias">
					<a href="{/ow:wiki/ow:scriptname}?{ow:urlencode(string(ow:by/@alias))}">
						<xsl:value-of select="ow:by/text()"/>
					</a>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template name="attachbody">
		<h1>Attachments for <xsl:value-of select="ow:page/@name"/>
		</h1>
		<hr size="1"/>
		<small>
            Back to <a href="{ow:wiki/ow:scriptname}?{$name}">
		...
		<hr size="1"/>
		<!-- MOD by Carl Leubsdorf: fake progress bar to display some kind of activity during long uploads -->
		...
		<span style="visibility:hidden;border-right: activeborder thin outset; border-top: activeborder thin outset; font-size: x-small; z-index: 1003;border-left: activeborder thin outset;border-bottom: activeborder thin outset;font-family: Verdana, Arial;position: absolute;border-collapse: collapse;background-color: #F0F0E9;width: 200px;	height: 50px;text-align: center;" ID="progressBar">
			...
			<OBJECT codeBase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,0,0" height="7" width="78" align="middle" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" ID="Shockwaveflash1">
				<PARAM NAME="Movie" VALUE="ow/images/loading.swf"/>
			</OBJECT>
		</span>
		...
		<form name="fup" method="post" action="{/ow:wiki/ow:scriptname}?p={$name}&amp;a=upload" enctype="multipart/form-data">
				<table cellspacing="0" cellpadding="2" border="0">
					...
					<tr>
						<td>&#160;</td>
						<td>
							<input type="checkbox" name="link" value="1"/>Create a link to the attached file at the end of the WikiPage.</td>
					</tr>
					<tr>
						<td>&#160;</td>
						<td>
							<input type="checkbox" name="hide" value="1" checked="checked"/>Hide the attached file in normal view.</td>
					</tr>
					<tr>
						<td>&#160;</td>
						<td>
	...
	<xsl:template match="/ow:wiki" mode="attachchanges">
		...
		<html>
			...
			<body bgcolor="#ffffff" onload="window.defaultStatus='{$brandingText}'">
				...
				<h1>History of <xsl:value-of select="ow:page/ow:attachments/ow:attachment/@name"/>
				</h1>
