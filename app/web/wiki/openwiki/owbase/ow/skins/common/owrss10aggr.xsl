<?xml version="1.0"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:ns="http://purl.org/rss/1.0/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:ag="http://purl.org/rss/1.0/modules/aggregation/" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:hr="http://www.w3.org/2000/08/w3c-synd/#" xmlns:wiki="http://purl.org/rss/1.0/modules/wiki/" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ow="http://openwiki.com/2001/OW/Wiki" extension-element-prefixes="rdf ns dc ag h hr wiki msxsl ow" exclude-result-prefixes="" version="1.0">
	<xsl:output method="xml" indent="no" omit-xml-declaration="yes"/>
	<xsl:include href="../common/owinc.xsl"/>
	<xsl:include href="../common/ow_JScripts.xsl"/>
			...
			<xsl:for-each select='ns:item[count(. | key("items-by-timestamp", substring-before(ag:timestamp, "T"))[1]) = 1]'>
				...
				<tr>
					<td colspan="5">
						<br/>
						<b>
							<xsl:value-of select="ow:formatLongDate(string(ag:timestamp))"/>
						</b>
					</td>
				</tr>
...
-->
						...
						<td colspan="2">
							<xsl:choose>
								<xsl:when test="ag:source/rdf:Description/@wiki:interwiki">
									<a href="{ag:sourceURL}" class="rss" title="{ag:source}">
										<xsl:value-of select="ag:source/rdf:Description/@wiki:interwiki"/>
									</a>:<a href="{ns:link}" class="rss" title="Version: {wiki:version}">
										<xsl:call-template name="truncatethis">
											<xsl:with-param name="thetext" select="string(ns:title)"/>
											<xsl:with-param name="syntax" select='string("2|....|1")'/>
										</xsl:call-template>
									</a>
								</xsl:when>
								<xsl:otherwise>
									<a href="{ns:link}" class="rss" title="Version: {wiki:version}">
						...
						<xsl:choose>
								...
								<td nowrap="nowrap" align="left">
									<xsl:choose>
										<xsl:when test="dc:contributor/rdf:Description/@link">
											<a href="{dc:contributor/rdf:Description/@link}" class="rss" title="{dc:contributor/rdf:Description/@wiki:host}">
												<xsl:value-of select="dc:contributor/rdf:Description/rdf:value"/>
											</a>
										</xsl:when>
										<xsl:otherwise>
											<xsl:choose>
							...
							<xsl:otherwise>
								<td align="right">
									...
									<a href="{ag:sourceURL}" class="rss">
										<xsl:value-of select="ag:source"/>
									</a>
								</td>
							</xsl:otherwise>
						</xsl:choose>
	...
	<xsl:template match="ns:image">
		<a href="{ns:link}" class="rss">
			<img src="{ns:url}" alt="{ns:title}" border="0"/>
		</a>
	</xsl:template>
	<xsl:template match="ns:textinput">
		<form action="{ns:link}" method="post">
			<xsl:value-of select="ns:title"/>:<br/>
