...
'	Date:		March 19, 2002
'   Mod:		Carl Leubsdorf, 14-August-2004, mods for use in OpenWiki
' ------------------------------------------------------------------------------
...
'
'		Use:
'			Set objUploadField = objUpload.Fields("File1")
'			Set objUploadField = objUpload("File1")
'			Set objUploadField = objUpload.Fields(0)
'			Set objUploadField = objUpload(0)
...
' ------------------------------------------------------------------------------
	...
	Private mobjUploadFieldAry()		' Array to hold field objects
...
' ------------------------------------------------------------------------------
	Private Sub AddField(ByRef pstrName, ByRef pstrFileName, ByRef pstrContentType, ByRef pstrValue, ByRef pbinData)
		...
		Dim lobjUploadField		' Field object class
		
		' Add a new index to the field array
		' Make certain not to destroy current fields
		ReDim Preserve mobjUploadFieldAry(mlngCount)

		' Create new field object
		Set lobjUploadField = New UploadField
		
		' Set field properties
		lobjUploadField.Name = pstrName
		lobjUploadField.FilePath = pstrFileName				
		lobjUploadField.ContentType = pstrContentType

		' If field is not a binary file
		If LenB(pbinData) = 0 Then
			
			lobjUploadField.BinaryData = ChrB(0)
			lobjUploadField.Value = pstrValue
			lobjUploadField.Length = Len(pstrValue)

		' Else field is a binary file
		Else

			lobjUploadField.BinaryData = pbinData
			lobjUploadField.Length = LenB(pbinData)
			lobjUploadField.Value = ""

		End If

		' Set field array index to new field
		Set mobjUploadFieldAry(mlngCount) = lobjUploadField
	...
	Public Default Property Get Fields(ByVal pstrName)
		...
		If IsNumeric(pstrName) Then
			...
			If llngIndex > mlngCount - 1 Or llngIndex < 0 Then
				' Raise an error
				Call Err.Raise(vbObjectError + 1, "owupload.asp", "Object does not exist within the ordinal reference.")
				Exit Property
			End If
				
			' Return the field class for the index specified
			Set Fields = mobjUploadFieldAry(pstrName)
		...
		Else
			...
			For llngIndex = 0 To mlngCount - 1
				...
				If LCase(mobjUploadFieldAry(llngIndex).Name) = pstrName Then
					
					' Return Field Class
					Set Fields = mobjUploadFieldAry(llngIndex)
					Exit Property
...
' ------------------------------------------------------------------------------
	Private Sub Class_Terminate()
		...
		For llngIndex = 0 To mlngCount - 1
			...
			Set mobjUploadFieldAry(llngIndex) = Nothing
			
		Next
		
		' Redimension array and remove all data within
		ReDim mobjUploadFieldAry(-1)
