...
<TABLE WIDTH=75% BORDER=0 CELLPADDING=0 CELLSPACING=4 bgcolor="#dcbace">  
	 ...
	 <tr>
		<td align="center" style="border:1px solid #dcbace;">
    		<FORM ENCTYPE="multipart/form-data" METHOD=post id=form1 name=form1 action="/seekers/registered/emailresume/upload.asp?func=2">
    			<br><br>
...
msgBody = "<!DOCTYPE HTML PUBLIC ""-//W3C//DTD HTML 4.0 Transitional//EN"">" & vbCrLf _
		...
		& " <p>" & vbCrLf _
		& user_lastname & ",&nbsp;" & user_firstname & "<BR>" & vbCrLf _
...
'	  .To = "saguilar@personnel.com"
      ...
      .Subject = "Personnel Plus - Email Resume" & " " & user_lastname & ", " & user_firstname
      .Send 
End With 
...
Set cdoConfig = Nothing 
    ...
    set fso = nothing
			...
			if session("state") = "ID" then
				'is IDAHO resident
				response.redirect("/seekers/registered/index.asp?who=2&resVal=Yes&resType=int&success=1")
			else
				' not a Idaho resident
				response.redirect("/seekers/registered/index.asp?who=2&resVal=Yes&resType=ext&success=1")
			end if
