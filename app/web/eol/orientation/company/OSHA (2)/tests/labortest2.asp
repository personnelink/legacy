...
<script>
  ...
  function init(){
    ...
    objXmlHTTP.open("POST","laborExam.asp?Action=Start",false);
    objXmlHTTP.send("");
    ...
    if(count < aQuest.length) {
      url = "laborExam.asp?Action=NextQ&QNo=" + aQuest[count];
  ...
  function showResult() {
    ...
    strRes += "<% response.write request.cookies("OSHA")("fname") %>&nbsp;<% response.write request.cookies("OSHA")("lname") %>,&nbsp;here are your Labor Test Results...<br>";
...
<body>
<h2 align=center><font color=green>Labor Orientation Test</font></h2>
