...
<% if session("adminAuth") = "true" then response.redirect("/lweb/admin/index.asp") end if%>
...
<!-- #INCLUDE VIRTUAL='/lweb/inc/updateresume.asp' -->
<% end if
...
rsMessages.Open "SELECT * FROM tbl_messages WHERE userName = '" & session("userName") & "' AND accountType='Seeker' AND msgRead='No' ORDER BY msgCreationDate DESC",Connect,3,3

set rsAllMessages = Server.CreateObject("ADODB.RecordSet")
rsAllMessages.Open "SELECT * FROM tbl_messages WHERE userName = '" & session("userName") & "' AND accountType='Seeker'",Connect,3,3

Set rsResumeCount = Connect.Execute("SELECT count(userName) AS theCount FROM tbl_resumes WHERE userName = '" & session("userName") & "'")

Set rstotalJobApps = Connect.Execute("SELECT count(userName) AS theCount FROM tbl_applications WHERE userName = '" & session("userName") & "'")
...
<TITLE><%=session("firstName")%>&nbsp;<%=session("lastName")%>, Welcome to Personnel Plus!</TITLE>
...
<% end if%>		
<p></p>		
<font size="2">Welcome, <STRONG><%=session("firstName")%>&nbsp;<%=session("lastName")%></STRONG> to Personnel Plus!<br> We <u>thank you</u> for taking the time to create your own personalized account with us!</font>
<p></p>
...
<% End If %>		
		<!--  resume end -->		
<% If session("userName") = "Spike" then %>	
		<!-- messages start -->							
	<TR>
		<TD>
...
<% do until rsMessages.EOF %>
                 <TR> 
                     <TD BGCOLOR="#f5f5dc"><A HREF="../../../seekers/registered/messages/viewMessage.asp?msgID=<%=rsMessages("MSGID")%>"><img src="/img/msgNew.gif" alt="" width="18" height="12" border="0"></A> <A HREF="../../../seekers/registered/messages/viewMessage.asp?msgID=<%=rsMessages("MSGID")%>"><%=rsMessages("msgSubject")%></A></TD>
...
<%=session("firstName")%>, you have applied for  <STRONG><%=rsTotalJobApps("theCount")%></STRONG> job<% if rsTotalJobApps("theCount") = 1 then %> <% else %>s <% end if %> since <%=FormatDateTime(rsResume("dateCreated"),1)%>.<BR>
