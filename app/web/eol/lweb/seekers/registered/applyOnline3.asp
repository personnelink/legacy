...
msgBody = msgBody & session("firstName") & " " & session("lastName") & chr(13)
msgBody = msgBody & session("addressOne") & chr(13)
if session("addressTwo") <> "" then 
msgBody = msgBody & session("addressTwo") & chr(13) 
end if
msgBody = msgBody & session("city") & ", " & session("state") & " " & session("zipCode") & chr(13)
...
'	  .bcc = "jwerrbach@personnel.com"
      .Subject = "Personnel Plus - Direct Application: " & session("lastName") & ", " & session("firstName")
      .TextBody = msgBody
      .Send 
End With 
...
' is IDAHO resident
response.redirect("/lweb/seekers/registered/index.asp?who=2&?resVal=Yes&resType=int")
else
' not a Idaho resident
response.redirect("/lweb/seekers/registered/index.asp?who=2&?resVal=Yes&resType=ext")
