...
if rsEmployerProfile.RecordCount < 1 then response.redirect("/lweb/index2.asp?who=1&error=1")
if rsEmployerProfile("password") <> password then response.redirect("/lweb/index2.asp?who=1&error=2")
if rsEmployerProfile("suspended") = "Yes" then response.redirect("/lweb/index2.asp?who=1&error=3")
...
session("companyName") = rsEmployerProfile("companyName")
...
session("zipCode") = rsEmployerProfile("zipCode")
session("country") = rsEmployerProfile("country")
session("companyPhone") = rsEmployerProfile("companyPhone")
...
session("employerAuth") = "true"
  ...
  response.cookies("employer").path = "/lweb/"
