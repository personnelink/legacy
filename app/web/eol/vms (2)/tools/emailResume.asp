<%Option Explicit%>
<!-- #INCLUDE VIRTUAL='/vms/include/master/masterPageTop.asp' -->
    <!-- Revised: 12.17.2008 -->
    <style type="text/css">
...
margin-bottom:1em;
height:2.5em;
background:url('/lweb/vms/include/style/images/normalTitleBackground.gif') repeat-x bottom;
}
fieldset
{
border: none;
...
%>
    <div id="uploadResumeForm" class="clearfix">
      ...
      <div class="mb clearfix">
        ...
        <% end if %>
        <form enctype="multipart/form-data" method=post id="form1" name="form1" action="/lweb/vms/tools/emailResume.asp?func=2">
          ...
          <ul>
            ...
            <li>
              ...
              <input name="zipcode" type="text" style="width:15em;height:1.5em;" value"<%=Session("zipcode")%>">
            </li>
          </ul>
    ...
    <!-- #INCLUDE VIRTUAL='/vms/include/master/pageFooter.asp' -->
<%
		case 2
			ForWriting = 2
    			adLongVarChar = 201
...
Loop
	...
	Set region = Database.Execute("Select address, city, state, zip From tbl_addresses Where addressID=" & Session("addressID"))
...
'	-- Error handling
    ...
    set fso = nothing
	response.redirect"/lweb/vms/tools/emailResume.asp?func=3"        		

Case 3
	Session("homeMessageHeading") = "Resume Submitted"
	Session("homeMessageBody") = "<strong> Thank you for keeping us updated!</strong>" &_
		"<p>Your resume has been placed into our applicant pool database so that we can better find you and match you up with job opportunities that meet " &_
	...
	Response.Redirect("/lweb/vms/userHome.asp")
End select %>
