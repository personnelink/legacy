...
session("additionalScripting") = "<script type=""text/javascript"" src=""/include/js/whoseHere.js""></script>"
%>
...
<form id="whoseHereForm" name="whoseHereForm" action="<%=aspPageName%>" method="post">
  ...
  <%
	if len(whichCompany & "") > 0 then
		...
		sqlWhichCustomer = "SELECT Orders.Customer FROM (Customers INNER JOIN Orders ON (Customers.Customer = Orders.Customer) " &_
			"AND (Customers.Customer = Orders.Customer)) " &_
			"LEFT JOIN OtherOrders ON Orders.Reference = OtherOrders.Reference WHERE (((Orders.JobStatus)<2)) " &_
	...
	'orderType
		...
		dim currentOrderType, OrderTypes(4), i
		response.write("<div id="""" class=""altNavPageRecords"">Click Order Type: <br>")
		OrderTypes(0) = "1Open<br>Nobody"
		OrderTypes(1) = "1Open<br>Unfilled"
		OrderTypes(2) = "3Open<br>Filled"
		OrderTypes(3) = "2Closed"
		OrderTypes(4) = "4To Be<br>Deleted"
		for i = 0 to 4
			response.write "<A HREF=""?WhichCustomer="& CurrentCustomer & "&WhichCompany=" & WhichCompany & """type=" & Left(OrderTypes(i),1) & """>&nbsp;"
			if Left(OrderTypes(i),1) = orderType then
				response.write "<span style=""color:red"">" & Right(OrderTypes(i), len(OrderTypes(i))-1) & "</span>"
			Else
				response.write Right(OrderTypes(i), len(OrderTypes(i))-1)
			end if
			response.write "&nbsp;</A>"
		next
		response.write("</div>")
		...
		do while not WhichCustomer.Eof
			...
			response.write "<A HREF=""?WhichCustomer="& CurrentCustomer & "&" & "WhichCompany=" & WhichCompany & """&type=" & orderType & ">&nbsp;"
