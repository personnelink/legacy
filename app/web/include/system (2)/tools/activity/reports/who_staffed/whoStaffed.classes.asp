...
class cApplicantSources
	...
	Sub Class_Initialize()
		set m_Sources = Server.CreateObject ("Scripting.Dictionary")
	End Sub
	Sub Class_Terminate()
		set m_Sources = Nothing
	End Sub

	public property get Sources()
	...
	public function LoadSources()
		...
		REM dim strDateWhere, strDateHaving
		REM if isDate(m_FromDate) then 'check if FromDate is valid
			REM if not isDate(m_ToDate) then
				REM ''From' date valid, check if 'To' date is also, otherwise set To date to todays
				REM m_ToDate = Date()
			REM end if
		REM elseif isDate(m_ToDate) then
			REM 'From date is invalid.
			REM 'Check 'To' date and if valid inputs are reversed; set 'From' date to today
			REM m_FromDate = Date()
		
		REM else
			REM 'Neither date is valid or set. Default variables to last 30 days.
			REM m_ToDate = Date()
			REM m_FromDate = DateAdd("d", -90, m_ToDate)
		REM end if
		REM strDateWhere = "(ARSummary.InvoiceDate BETWEEN #" & m_FromDate & "# AND #" & m_ToDate & "#)"
		REM strDateHaving = "(HistoryDetail.InvoiceDate BETWEEN #" & m_FromDate & "# AND #" & m_ToDate & "#)"


 		 strSQL = "" &_
			"SELECT DISTINCT who_staffed AS whoStaffed, COUNT(who_staffed) AS HowOften, MONTH(modifiedDate) AS MonthLastActive, YEAR(modifiedDate) AS YearLastActive " &_
			"FROM pplusvms.tbl_applications WHERE who_staffed IS NOT NULL " &_
			"GROUP BY who_staffed, MONTH(modifiedDate), YEAR(modifiedDate) " &_
			"ORDER By YEAR(modifiedDate) desc, MONTH(modifiedDate) desc, COUNT(who_staffed) desc, who_staffed;"
			'print strSQL

		LoadSources = LoadData (strSQL)
	...
	public function GetPageSelection()
		...
		p_strPageSelection = "<div id=""topPageRecords"" class=""navPageRecords"">" &_
			...
			Next
		p_strPageSelection = p_strPageSelection &_
			"<A HREF=""javascript:;"" onclick=""setpage('" & m_PageCount & "')"">Last</A>" &_
			"</div>"
    ...
    Private Function FillFromRS(p_RS)
		...
		dim thisSource
 		do while not ( p_RS.eof Or p_RS.AbsolutePage <> m_Page )
			set thisSource = New cApplicantSource
			p_Key = p_Key + 1
			with thisSource
				.Id              = p_Key : p_Key = p_Key + 1
				.whoStaffed     = p_RS.fields("whoStaffed").Value
				.HowOften  = p_RS.fields("HowOften").Value
				'.CityStateZip       = p_RS.fields("CityStateZip").Value
				.MonthLastActive       = p_RS.fields("MonthLastActive").Value
				.YearLastActive        = p_RS.fields("YearLastActive").Value
			end with
			m_Sources.Add thisSource.Id, thisSource
			p_RS.movenext
        loop
	End Function
...
class cApplicantSource
	private m_Id
	private m_whoStaffed
	private m_HowOften
	private m_CityStateZip
	private m_MonthLastActive
	private m_YearLastActive
	...
	public property get whoStaffed()
		whoStaffed= m_whoStaffed
	end property
	public property let whoStaffed(p_whoStaffed)
		m_whoStaffed = p_whoStaffed
	end property
	
	public property get HowOften()
		HowOften = m_HowOften
	end property
	public property let HowOften(p_HowOften)
		m_HowOften = p_HowOften
	end property
	
	public property get MonthLastActive()
		MonthLastActive = m_MonthLastActive
	end property
	public property let MonthLastActive(p_MonthLastActive)
		m_MonthLastActive = p_MonthLastActive
	end property
	
	public property get YearLastActive()
		YearLastActive	= m_YearLastActive
	end property
	public property let YearLastActive(p_YearLastActive)
		m_YearLastActive = p_YearLastActive
	end property
