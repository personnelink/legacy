...
function makeSideMenu ()
	...
	strTmp = "<div id=""what_activities"" class=""navPageRecords"">" &_
			"<p><strong>Activities to include in report:</strong></p>" &_
			"<div id=""nonsystem"">" &_
	...
	if ThisSecurityLevel => userLevelPPlusStaff then
		strTmp = strTmp &_	
			"</ul></div>" &_
			"<p><strong>Some additional system specific activities:</strong></p>" &_
			"<div><ul class=""what_activities alignL"">"
	...
	htmlDivMenu = replace(strTmp, "</form>", "")
...
function navChooseCustomer (whichCompany)
	if ThisSecurityLevel => userLevelPPlusStaff then
		if len(whichCompany & "") > 0 then
			...
			if not prospecting then
				sqlWhichCustomer = "" &_
					"SELECT DISTINCT Orders.Customer, Customers.CustomerName " &_
					...
					"INNER JOIN Appointments ON Customers.Customer = Appointments.Customer " &_
					"WHERE (((Orders.JobStatus)<4) AND ((Customers.CustomerType)<>""P"" Or (Customers.CustomerType)<>""H"")) " &_
				...
				sqlWhichCustomer = "" &_
					...
					"LEFT JOIN OtherOrders ON Orders.Reference = OtherOrders.Reference) " &_
					"INNER JOIN Appointments ON Customers.Customer = Appointments.Customer " &_
					"WHERE ((Customers.CustomerType)<>""H"") " &_
					"ORDER BY Customers.CustomerName;"
			end if
...
function memo_row (strActivityBlob)
	memo_row = "<table style='width:100%'><tr>" & "<td>" & strActivityBlob & "</td>" & "</tr></table>"	
end function
...
if len(whichCompany & "") > 0 and len(include_what) > 0 then
	...
	With rsWhoseHere
		...
		case true
			...
			' salvaged below query, with the flopped customer relationship, because it could be useful later as what it ends up doing it returning
