...
var getInvoice = {
	open: function (row_number, inv_number, site, cust) {
		...
		this_row.innerHTML = "<a href=\"javascript:;\" onclick=\"getInvoice.close('"+row_number+"', '"+inv_number+"', '"+site+"', '"+cust+"');\">"+inv_number+" -</a>";
	...
	close: function (row_number, inv_number, site, cust) {
		...
		var this_row = document.getElementById("row_for_inv_"+inv_number);
		this_row.innerHTML = "<a href=\"javascript:;\" onclick=\"getInvoice.open('"+row_number+"', '"+inv_number+"', '"+site+"', '"+cust+"');\">"+inv_number+" +</a>";
		return false;
	}
}
...
var look_up_inv = function(row_number, inv_number, site, cust) {
	...
	var PostStr = "inv=" + inv_number + "&" + site + cust;
	
	// use the generic function to make the request
	doAJAXCall('/include/system/tools/reports/ajax/getInvoice.asp?'+PostStr, 'POST', '' + PostStr + '', showMessageResponse);
	return false;
}
