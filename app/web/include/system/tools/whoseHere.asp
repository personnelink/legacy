...
<form id="whoseHereForm" name="whoseHereForm" action="<%=aspPageName%>" method="post">
  ...
  <p>
  	<a id="lookup" href="#" onclick="lookup_id()">&nbsp;</a>
	<label for="signApplicantIn">Applicant Id:<br />
		<input name="signApplicantIn" id="signApplicantIn" value="<%=who%>" /></label>
	<label for="signInComment">Comments:<br />
		<input name="signInComment" id="signInComment" value="<%=signInComment%>" /></label>
		<a id="sign_in" class="squarebutton" href="#" onclick="javascript:document.whoseHereForm.action.value='signin';document.whoseHereForm.submit();"><span>Sign-In</span></a>
	</p>
  ...
  <%
	...
	response.write("<div id=""lookup_applicant"" class=""show"">")
		Response.Write("<p>Applicant Lookup</p>")
	response.write("</div>")
	...
	if not WhoseHere.eof then WhoseHere.AbsolutePage = nPage
		...
		tableHeader = "<table class=""generalTable""" &_
