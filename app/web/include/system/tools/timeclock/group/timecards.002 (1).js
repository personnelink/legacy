...
var timedetail = {
	...
	addRow: function (tableID) {
		...
		// get 'input' tags
		var subInputs = divTimeDetail.getElementsByTagName('input');
		  for(var i=0;i<subInputs.length;i++) {
			ajaxTimeDetailBuffer[0][i] = subInputs[i].id;
			ajaxTimeDetailBuffer[1][i] = subInputs[i].value;
			
			console.log('input id:'+subInputs[i].id+',value:'+subInputs[i].value);
		  }
		  var slctOffset = i;
		  
		  // get 'select' tags
		  var subSelects = divTimeDetail.getElementsByTagName('select');
	...
	appendRow: function(oXML) {
		...
		for(var i=0;i<ajaxTimeDetailBuffer[0].length;i++) {
			document.getElementById(ajaxTimeDetailBuffer[0][i]).value=ajaxTimeDetailBuffer[1][i];
		}
	},
...
var changeAudit = {
		compare: function(objAudit) {
			...
			if (cid!==sid) {
				var changedWhy=prompt("Please enter reason for change","Missed swipe");
				...
				var PostStr = "do=audittimechange" +
					...
					"&why="+encodeURIComponent(changedWhy) +
					"&site=na";
				
				console.log(changedWhy);
...
var timeclock = {
	setTime: function(id) {
		...
		switch(idtype){
			case "in":
				console.log("in");
				objTimeChangeAudit.pvalue=timein;
			...
			case "out":
				console.log("out");
				objTimeChangeAudit.pvalue=timeout;
				if (changeAudit.compare(objTimeChangeAudit) === true) {
