<%
session("add_css") = "./whoStaffed.css"
session("required_user_level") = 2048 'userLevelSupervisor
session("window_page_title") = "Who Staffed - Personnel Plus"
%>
<!-- #INCLUDE VIRTUAL='/include/core/init_secure_session.asp' -->
<% if isDev then Server.ScriptTimeout = 3 %>

<script type="text/javascript" src="whoStaffed.js"></script>
<!-- #include file='whoStaffed.doStuff.asp' -->

<!-- begin presentation stuff -->
<form id="report_form" name="report_form" action="<%=aspPageName%>" method="post">
<%=decorateTop("track_activities", "notToShort marLR10", "Companies Applicants were sent to by other Staffing Agencies")%>
...
%>		<table class="account_activity departmenttotal">
				<tr>
					<th class="">Staffing Agency Placed Clients</th>
...
%>
	<tr class="<%=row_shade%>">
		<td id="source" class=""><span style="font-weight:normal;" onclick="source.edit('<%=Source.Id%>');" id="from<%=Source.Id%>"><%=Source.whoStaffed%></span>
