...
if isDev then Server.ScriptTimeout = 3
	...
	if this_site > -1 then
		...
		With getAccountActivity_cmd
			...
			.CommandText = "SELECT HistoryDetail.WorkDate, HistoryDetail.Customer, " &_
		...
		'print getAccountActivity_cmd.CommandText
		...
		dim expenseType     : expenseType     = ""
		
		do while not AccountActivity.eof
			
			expenseType = AccountActivity.Fields.Item("Type")
			select case expenseType
			case "4"
				dblBillrate = cdbl(AccountActivity.Fields.Item("Billrate"))
				invTotalDollars = invTotalDollars + dblBillrate
				expenseType = "E"
			...
			Response.write "" &_
				"<tr class=""" & row_shade & """>" &_
					...
					"<td class=""be"" title=""'E' - Expense, 'O' - Overtime, 'R' - Regular"">" & expenseType & "</td>" &_
		...
		Response.write "" &_
			"<tr>" &_
				...
				"<th class=""tDollars"" title=""Invoice #" & which_invoice & " Total"">$" & TwoDecimals(invTotalDollars+0.005) & "</td>" &_
			"</tr>"
