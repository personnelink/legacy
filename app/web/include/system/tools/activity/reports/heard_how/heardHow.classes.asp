...
class cApplicantSources
	...
	public function LoadAccountActivity()
		...
		dim strSQL
 		' strSQL = "" &_
 		 ...
 		 strSQL = "" &_
			"SELECT DISTINCT lcase(heardAboutUs), COUNT(heardAboutUs) AS HowOften, MONTH(modifiedDate), YEAR(modifiedDate) " &_
			"FROM pplusvms.tbl_applications WHERE heardAboutUs IS NOT NULL " &_
			"GROUP BY heardAboutUs, city, appState, zipcode, MONTH(modifiedDate), YEAR(modifiedDate) " &_
			"ORDER By YEAR(modifiedDate) desc, MONTH(modifiedDate) desc, COUNT(heardAboutUs) desc, heardAboutUs;"
			'print strSQL
	...
	public function LoadSources()
		...
		REM strDateHaving = "(HistoryDetail.InvoiceDate BETWEEN #" & m_FromDate & "# AND #" & m_ToDate & "#)"
 		 ...
 		 strSQL = "" &_
			"SELECT DISTINCT lcase(heardAboutUs) AS HeardHow, COUNT(heardAboutUs) AS HowOften, MONTH(modifiedDate) AS MonthLastActive, YEAR(modifiedDate) AS YearLastActive " &_
			"FROM pplusvms.tbl_applications WHERE heardAboutUs IS NOT NULL " &_
			"GROUP BY heardAboutUs, city, appState, zipcode, MONTH(modifiedDate), YEAR(modifiedDate) " &_
			"ORDER By YEAR(modifiedDate) desc, MONTH(modifiedDate) desc, COUNT(heardAboutUs) desc, heardAboutUs;"
			'print strSQL
		...
		dim thisSource
 		do while not ( p_RS.eof Or p_RS.AbsolutePage <> m_Page )
			...
			with thisSource
				...
				.HeardAboutUs     = p_RS.fields("HeardHow").Value
...
class cApplicantSource
	...
	public property get HeardAboutUs()
		HeardAboutUs= m_HeardAboutUs
	end property
	public property let HeardAboutUs(p_HeardAboutUs)
		m_HeardAboutUs = p_HeardAboutUs
	end property
