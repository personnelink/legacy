...
var orders = {
	get: function(id, jobstatus) {
		//set spinning busy image...
		document.getElementById(id+jobstatus).className = "loadingorders";

		var site = document.getElementById("whichCompany").value;

		//do procedure call
		var PostStr = "";
		doAJAXCall('ajax/doThings.asp?get=customers&do=view&id='+id+'&site='+site+'&status='+jobstatus, 'POST', '' + PostStr + '', orders.show);
	},
	hide: function(id) {
	...
	show: function(oXML) {
		...
		token = response.indexOf("[:]"), response.indexOf("&amp;action"));
		token = response.slice(response.indexOf("('appID=")+8, response.indexOf("&amp;action"));
	...
	var id = oXML.responseText;
		...
		document.getElementById("loader"+id).className = "idle";
