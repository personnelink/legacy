...
var user_info = {
	lookup: function () {
		...
		else if (search==='gus'){search='%000020021PERPERSON;';} // me test user
		...
		if (isNaN(applicant)!=true){
			...
			window.location.replace("https://www.personnelinc.com/time/clock/check/?"+GetStr);
...
var cost_centers = {
	...
	get_actions: function(placementid, applicantid, siteid, customer){
		...
		window.location.replace("https://www.personnelinc.com/time/clock/check/in/?"+GetStr);
...
var timeclock = {
	show_actions: function (oXML) {
		...
		console.log('check clock in or out')
		if (response.indexOf("You have successfully clocked <em>out</em>")>0){
			console.log('clock out sound');
			PlaySound("clock_out_sound");
		} else if (response.indexOf("You have successfully clocked <em>in</em>") > 0){
			console.log('clock in sound');
			PlaySound("clock_in_sound");
		
		} else {
			console.log('neither clock in')
		}
...
$(document).ready(function () {
	...
	try {
		var action_status = document.getElementById("clock_action_results").innerHTML;
		var needs_reset = action_status.indexOf("You have successfully clocked");
		console.log('action_status:'+needs_reset);
		if(needs_reset>0){
			window.setTimeout(function(){window.location.replace("https://www.personnelinc.com/time/clock/")},1000);
		}
	} catch (e) {
		console.log ('an error happened '+e);
	}
	
	var default_cost_center = $( ".defaultCostCenter" ).length;
	
	console.log("defaultCostCenter: "+default_cost_center);
	if (!!default_cost_center){
		
		window.setTimeout(function(){$( ".defaultCostCenter" ).trigger("click");},3000);

		console.log('default cost center found!');
	} else {
		console.log('current:'+window.location.href);
		if (window.location.href.indexOf("/time/clock/check/cost_centers/?")>0 | window.location.href.indexOf("/time/clock/check/?")>0){
			console.log('no default cost center found');
			window.setTimeout(function(){window.location.replace("https://www.personnelinc.com/time/clock/");},5000);
		}
	}
