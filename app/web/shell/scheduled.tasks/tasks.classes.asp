...
class cAlarm
	...
	private m_NumOfAppoinments
	private m_AppointmentNumber
	private m_Site
	private m_ForUser
	private m_Appointment

	Sub Class_Initialize()
        set m_Appointment = New cAppointment
	End Sub
	Sub Class_Terminate()
		set m_Appointment = Nothing
	End Sub
	
	public property get Appointment()
		set Appointment = m_Appointment
	end property
	
	public property get Icon()
		
		if len(m_Icon) = 0 then
			m_Icon = "/include/images/remind.png"
		end if
		Icon = m_Icon
		
	end property
	public property let Icon(p_Icon)
		m_Icon = p_Icon
	end property
	...
	public property get NumOfAppointments()
		NumOfAppointments = m_NumOfAppoinments
	end property
	public property let NumOfAppointments(p_NumOfAppointments)
		m_NumOfAppointments = p_NumOfAppointments
	end property
	
	public property get AppointmentNumber()
		AppointmentNumber = m_AppointmentNumber
	end property
	public property let AppointmentNumber(p_AppointmentNumber)
		m_AppointmentNumber = p_AppointmentNumber
	end property
	
	public property get Site()

		if len(m_Site) = 0 then
			m_Site = Request.QueryString("site")
			if len(m_Site) = 0 then
				m_Site = request.form("site")
				if len(m_Site) = 0 then
					m_Site = session("location")
				end if
			end if
		end if
	...
	public property get ForUser()

		if len(m_ForUser) = 0 then
			m_ForUser = Request.QueryString("foruser")
			if len(m_ForUser) = 0 then
				m_ForUser = request.form("foruser")
				if len(m_ForUser) = 0 then
					m_ForUser = session("location")
				end if
			end if
		end if
	...
	public property let ForUser(p_ForUser)
		m_ForUser = p_ForUser
	end property
...
'#############  Public Functions ##############
	...
	public function GetAppointment()
		...
		'Appointments.DispTypeCode = 0 'Only Active Appointments
		strWhere = "((ApptTypes.ApptTypeCode=0 OR ApptTypes.ApptTypeCode=4 OR ApptTypes.ApptTypeCode=6) AND Appointments.DispTypeCode=0) "
		
		if len(m_ForUser) > 0 then
			strWhere = "(" & strWhere & "AND (AssignedTo='" & m_ForUser & "' OR (AssignedTo='{anyone}' AND EnteredBy='" & m_ForUser &"'))) "
		end if
		...
		GetAllPlacements = LoadAppointmentData (strSQL)
    ...
    private function FillAppointmentFromRS(p_RS)
		p_RS.PageSize = 1
		m_PageCount = p_RS.PageCount
		m_NumOfAppoinments = p_RS.PageCount
				
		if m_Page < 1 Or m_Page > m_PageCount then
			m_Page = 1
		else
			m_Page = AppointmentNumber
		end if
		...
		if not ( p_RS.eof Or p_RS.AbsolutePage <> m_Page ) then
			with m_Appointment
				...
				.Comment           = p_RS.fields("Comment").Value
				.ApplicantPhone    = p_RS.fields("TelePhone").Value
				.CustomerPhone     = p_RS.fields("Phone").Value
				.WorkSitePhone     = p_RS.fields("WorkSite3").Value
				.ApplicantId       = p_RS.fields("ApplicantID").Value
				.LastnameFirst     = p_RS.fields("LastnameFirst").Value
				.ApptDate          = p_RS.fields("AppDate").Value
				.ApptType          = p_RS.fields("ApptType").Value
				.ApptTypeCode      = p_RS.fields("ApptTypeCode").Value
				.Disposition       = p_RS.fields("Disposition").Value
				.DispTypeCode      = p_RS.fields("DispTypeCode").Value
				.EnteredBy         = p_RS.fields("EnteredBy").Value
				.AssignedTo        = p_RS.fields("AssignedTo").Value
				.CustomerCode      = p_RS.fields("Customer").Value
				.CustomerName      = p_RS.fields("CustomerName").Value
				.CustomerContact   = p_RS.fields("Contact").Value
				.JobSupervisor     = p_RS.fields("JobSupervisor").Value
				.ReferenceId       = p_RS.fields("Reference").Value
				.JobDescription    = p_RS.fields("JobDescription").Value
			end with
		end if
    end function

    private function LoadAppointmentData(p_strSQL)
        dim rs
        ...
        FillAppointmentFromRS(rs)
        LoadAppointmentData = rs.recordcount
        rs. close
        set rs = nothing
    End Function
end class
...
class cAppointment
	...
	public property get ApptType()
		dim ChangeAppointment, CurrentAppointment(18)
...
class cPlacements
	...
	public function GetAllPlacements()
		...
		if onlyactive then
			...
			strWhere = "(ApptTypes.ApptTypeCode=0 OR ApptTypes.ApptTypeCode=4 OR ApptTypes.ApptTypeCode=6) "
		end if
		if len(foruser) > 0 then
			strWhere = "(" & strWhere & "AND (AssignedTo='" & foruser & "' OR (AssignedTo='{anyone}' AND EnteredBy='" & foruser &"'))) "
		end if
		if is_service then
			strWhere = "(" & strWhere & "AND AppDate <= #" & Date() & "#) "
		end if
