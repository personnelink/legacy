	...
	'#############  Public Functions ##############
		...
		public function GetCustomers()
			...
			elseif len(me.Customer) > 0 then
				strSelectCriteria = "WHERE (Customers.Customer=""" & me.Customer & """)"
			end if

			dim strSql
			strSql = "" &_
				"SELECT Customers.Customer AS CustomerCode, Customers.CustomerName, Customers.Address, Customers.Cityline, " &_ 
				"Customers.Contact, Customers.Phone, Customers.DateLastActive " &_
				"FROM Customers " &_
				strSelectCriteria & ";"
...
class cPlacement
	...
	public sub Class_Initialize
		Randomize
		if Int((Rnd * 1) + 0.5) > 0 then
			m_TotalTime = int(Rnd * 80)
		else
			m_TotalTime = 0
		end if
		
		if Int((Rnd * 1) + 0.5) > 0 then 
			m_TotalExpenses = Rnd * 100
		else
			m_TotalExpenses = 0
		end if
	end sub
